...
@section('title', 'Edit Zona') @section('page-title', 'Edit Zona: ' . $zone->name)
@section('content')
<div class="max-w-lg">
    <div class="bg-[var(--color-surface-800)] rounded-2xl border border-[var(--color-surface-600)] p-6">
        <form method="POST" action="{{ route('zones.update', $zone->id) }}">
            ...
            <div class="mb-5">
                ...
                <select id="operation_id" name="operation_id" required
                    ...
                    @foreach($operations as $op)
                        <option value="{{ $op->id }}" @selected(old('operation_id', $zone->operation_id) === $op->id)>
            ...
            <div class="mb-5">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $zone->name) }}" required maxlength="150"
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="3" maxlength="500"
                          class="w-full px-4 py-3 bg-[var(--color-surface-700)] border border-[var(--color-surface-500)] rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] resize-none">{{ old('description', $zone->description) }}</textarea>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="flex-1 px-4 py-3 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white text-sm font-medium rounded-xl transition-colors cursor-pointer">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
