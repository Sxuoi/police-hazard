...
<body
    class="min-h-screen bg-surface-900 text-gray-200 antialiased"
...
>
    ...
    {{-- Sidebar --}}
    <aside
        class="fixed top-0 left-0 z-40 h-screen w-64 bg-surface-800 border-r border-surface-600 transition-transform duration-200 flex flex-col"
        :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
    >
        {{-- Brand --}}
        <div class="h-16 flex items-center gap-3 px-5 border-b border-surface-600">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1" x-data="{ openMenu: {{ $defaultMenu }} }">
            ...
            <div class="mb-2">
                <button @click="openMenu = openMenu === 'police_hazard' ? '' : 'police_hazard'"
                    class="w-full flex items-center justify-between px-3 py-2 text-sm font-medium rounded-lg transition-colors {{ $isPoliceHazardActive ? 'bg-surface-600 text-white' : 'text-gray-400 hover:text-white hover:bg-surface-600' }}">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 {{ $isPoliceHazardActive ? 'text-info' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
                ...
                <div x-show="openMenu === 'police_hazard'" 
                     ...
                     class="overflow-hidden pl-5 pr-2 py-1 space-y-1 mt-1 border-l-2 border-surface-600 ml-5">
            ...
            <div class="mb-2">
                <button @click="openMenu = openMenu === 'layanan_110' ? '' : 'layanan_110'"
                    class="w-full flex items-center justify-between px-3 py-2 text-sm font-medium rounded-lg transition-colors {{ $isLayanan110Active ? 'bg-surface-600 text-white' : 'text-gray-400 hover:text-white hover:bg-surface-600' }}">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 {{ $isLayanan110Active ? 'text-info' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
            ...
            <x-sidebar-item href="{{ route('bypass-approvals.index') }}" icon="shield" :active="request()->routeIs('bypass-approvals.*')">
                <span class="flex items-center justify-between w-full">
                    ...
                    @if($pendingBypassCount > 0)
                        <span class="ml-2 inline-flex items-center justify-center min-w-5 h-5 px-1.5 rounded-full bg-red-500 text-white text-xs font-bold">
        ...
        <div class="border-t border-surface-600 p-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-accent/20 flex items-center justify-center text-sm font-medium text-accent">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" title="Logout" class="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-surface-600 transition-colors cursor-pointer">
    ...
    <div class="lg:pl-64 min-h-screen flex flex-col">
        {{-- Top Bar --}}
        <header class="sticky top-0 z-20 h-16 bg-surface-800/80 backdrop-blur-xl border-b border-surface-600 flex items-center justify-between px-4 lg:px-6">
            {{-- Mobile hamburger --}}
            <button @click="sidebarOpen = !sidebarOpen" class="lg:hidden p-2 rounded-lg text-gray-400 hover:text-white hover:bg-surface-600 transition-colors cursor-pointer">
            ...
            <div class="flex items-center gap-3">
                {{-- Saker Badge --}}
                <span class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent/10 text-xs font-medium text-accent">
                ...
                {{-- Notification Bell --}}
                <button class="relative p-2 rounded-lg text-gray-400 hover:text-white hover:bg-surface-600 transition-colors cursor-pointer">
